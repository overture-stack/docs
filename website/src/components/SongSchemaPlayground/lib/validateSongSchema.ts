...
const ALLOWED_SCHEMA_KEYS = new Set(['type', 'definitions', 'properties', 'required']);
...
function countPhrase(min: unknown, max: unknown, unit: string): string | null {
	...
	if (hasMin && hasMax) {
		return min === max ? `exactly ${pluralize(min, unit)}` : `${min} to ${pluralize(max, unit)}`;
	}
...
function resolvePointer(root: Record<string, unknown>, ref: string): Record<string, unknown> | null {
...
function flatten(def: Record<string, unknown>, ctx: BuildContext): Flattened {
	...
	const absorb = (node: Record<string, unknown>): void => {
		for (const [key, value] of Object.entries(node)) {
			if (key === 'properties' || key === 'required' || key === '$ref' || key === 'allOf') continue;
...
function describeField(flat: Flattened, ctx: BuildContext): FieldDescription {
	...
	if (isPlainObject(kw.not) && Object.keys(kw.not).length === 0) parts.push('forbidden');
...
function describeObject(flat: Flattened, ctx: BuildContext): string[] {
	...
	if (isPlainObject(kw.propertyNames)) {
		...
		if (Array.isArray(names.enum)) notes.push(`allowed keys: ${names.enum.map(literal).join(', ')}`);
		if (typeof names.pattern === 'string') notes.push(`keys must match ${names.pattern}`);
	}
	if (kw.additionalProperties === false) notes.push('no keys beyond those listed');
	if (isPlainObject(kw.additionalProperties)) {
		notes.push(`any other key must be ${typeLabel(flatten(kw.additionalProperties, ctx))}`);
	}
 ...
 */
function conditionParts(node: Record<string, unknown>, prefix: string, out: Condition[]): void {
	if (isPlainObject(node.properties)) {
		for (const [key, cond] of Object.entries(node.properties)) {
			if (!isPlainObject(cond)) continue;
			const path = prefix ? `${prefix}.${key}` : key;
			if (cond.const !== undefined) {
				out.push({ positive: `${path} is ${literal(cond.const)}`, negative: `${path} is not ${literal(cond.const)}` });
			} else if (Array.isArray(cond.enum)) {
				const values = joinValues(cond.enum, 3);
				out.push({ positive: `${path} is one of ${values}`, negative: `${path} is none of ${values}` });
			} else if (typeof cond.pattern === 'string') {
				out.push({ positive: `${path} matches ${cond.pattern}`, negative: `${path} does not match ${cond.pattern}` });
			} else if (isPlainObject(cond.properties)) {
				conditionParts(cond, path, out);
			} else {
				out.push({ positive: `${path} is set`, negative: `${path} is not set` });
			}
		}
	}
...
function branchDemands(node: Record<string, unknown>): BranchDemands {
	...
	if (isPlainObject(node.properties)) {
		for (const [key, sub] of Object.entries(node.properties)) {
			...
			else if (Array.isArray(sub.enum)) label = `one of ${joinValues(sub.enum, 3)}`;
			else if (Array.isArray(sub.oneOf)) label = sub.oneOf.map(branchLabel).join(' | ');
			else if (typeof sub.pattern === 'string') label = `matches ${sub.pattern}`;
...
function collectConditions(
	...
	if (isPlainObject(def.if) && (isPlainObject(def.then) || isPlainObject(def.else))) {
		const parts: Condition[] = [];
		conditionParts(def.if, '', parts);
		const positive = parts.map((p) => p.positive).join(' and ') || 'the if clause matches';
		...
		const negative = parts.length === 1 ? parts[0].negative : `not all of (${positive})`;

		if (isPlainObject(def.then)) {
			const demands = branchDemands(def.then);
			if (demands.text) {
				out.push({ line: `when ${[...when, positive].join(' and ')}: ${demands.text}`, requires: demands.required });
			}
		...
		if (isPlainObject(def.else)) {
			...
			if (demands.text) {
				out.push({ line: `when ${[...when, negative].join(' and ')}: ${demands.text}`, requires: demands.required });
			}
		...
		for (const [key, dependency] of Object.entries(def.dependencies)) {
			const names = Array.isArray(dependency)
				...
				: isPlainObject(dependency)
					...
					: [];
			if (names.length > 0) {
				out.push({ line: `when ${key} is present: requires ${nameList(names)}`, requires: names });
			}
		}
	}
}
...
function summarize(fields: PreviewField[]): string | null {
	...
	const parts: string[] = [];
	const required = fields.filter((f) => f.requirement === 'required').map((f) => f.name);
	const conditional = fields.filter((f) => f.requirement === 'conditional').map((f) => f.name);
	const optional = fields.filter((f) => f.requirement === 'optional').map((f) => f.name);
	if (required.length > 0) parts.push(`Requires ${nameList(required)}`);
	if (conditional.length > 0) parts.push(`Conditionally requires ${nameList(conditional)}`);
...
function addGroup(def: Record<string, unknown>, ctx: BuildContext, label: GroupLabel, depth: number): void {
	...
	const group: PreviewGroup = {
		...
		description: depth > 0 && typeof flat.keywords.description === 'string' ? flat.keywords.description : '',
	...
	for (const [key, rawField] of Object.entries(flat.properties)) {
		const requirement: Requirement = flat.required.has(key)
			...
			: conditionalRules.has(key)
				? 'conditional'
				: 'optional';
		const rule = requirement === 'conditional' ? (conditionalRules.get(key) ?? null) : null;
...
function buildPreview(name: unknown, schema: Record<string, unknown>, unresolved: Set<string>): PreviewSchema {
	const ctx: BuildContext = { root: schema, unresolved, groups: [] };
	const title = typeof name === 'string' && name.trim() ? name : '(unnamed)';
	addGroup(schema, ctx, { path: '', title, prefix: '', kind: 'analysis type', required: false }, 0);
 ...
 */
function conditionalRequirements(def: Record<string, unknown>, depth: number, out: Set<string>): void {
	if (depth > MAX_DEPTH) return;
...
function collectWarnings(
	...
	for (const [key, def] of Object.entries(properties)) {
		...
		if (!isPlainObject(def)) {
			if (!reported.has(path)) warnings.push(`property "${path}" should be an object.`);
			continue;
		}
		...
		for (const t of types) {
			if (!KNOWN_TYPES.has(t as string)) {
				warnings.push(`property "${path}": unknown type "${String(t)}".`);
			}
		}
		if ('enum' in def && !Array.isArray(def.enum)) {
			warnings.push(`property "${path}": enum should be an array of allowed values.`);
		}
		if (typeof def.pattern === 'string') {
			try {
				new RegExp(def.pattern);
			} catch {
				warnings.push(`property "${path}": pattern is not a valid regular expression.`);
			}
		}
		for (const numKey of ['minItems', 'maxItems', 'minimum', 'maximum'] as const) {
			if (numKey in def && typeof def[numKey] !== 'number') {
				warnings.push(`property "${path}": ${numKey} should be a number.`);
			}
		}
		...
		if (isPlainObject(def.properties)) {
			const defined = def.properties;
			for (const name of stringItems(def.required)) {
				if (!(name in defined)) {
					warnings.push(`"${path}" requires "${name}", which it does not define in properties.`);
				}
			}
			const conditional = new Set<string>();
			conditionalRequirements(def, 0, conditional);
			const undeclared = Array.from(conditional).filter((name) => !(name in defined));
			if (undeclared.length > 0) {
				warnings.push(
					`"${path}" conditionally requires ${quoteList(undeclared)}, which it does not define in properties; ` +
						'Song will check only that the key is present, not its value.',
				);
			}
			collectWarnings(defined, `${path}.`, warnings, depth + 1, reported);
		} else if (isPlainObject(def.items) && isPlainObject(def.items.properties)) {
			collectWarnings(def.items.properties, `${path}.items.`, warnings, depth + 2, reported);
		}
	}
}

function fail(...errors: string[]): SongValidationResult {
	return { status: 'error', errors, warnings: [], preview: null };
}
...
export function validateSongSchema(text: string): SongValidationResult {
	...
	try {
		...
		return fail(`JSON syntax error: ${e instanceof Error ? e.message : String(e)}`);
	}

	if (!isPlainObject(parsed)) {
		return fail('Top-level value must be a JSON object with "name" and "schema".');
	}
	if (!isPlainObject(parsed.schema)) {
		return fail('Missing "schema": a Song schema wraps its definition in a "schema" object.');
	}
	const schema = parsed.schema;
	if (!isPlainObject(schema.properties)) {
		return fail('schema.properties must be an object with at least one property.');
	}
	...
	if (typeof parsed.name !== 'string' || !parsed.name.trim()) {
		errors.push('Missing "name": Song registers the analysis type under this name.');
	} else if (!NAME_PATTERN.test(parsed.name)) {
		errors.push(`name "${parsed.name}": use only letters, numbers, dots, hyphens, and underscores.`);
	} else if (parsed.name === RESERVED_NAME) {
		errors.push(`name "${RESERVED_NAME}" is reserved by Song's registration endpoint; choose another name.`);
	}

	const strayEnvelopeKeys = Object.keys(parsed).filter((k) => !ALLOWED_ENVELOPE_KEYS.has(k));
	if (strayEnvelopeKeys.length > 0) {
		warnings.push(
			`${quoteList(strayEnvelopeKeys)} ${strayEnvelopeKeys.length === 1 ? 'is' : 'are'} not part of a registration ` +
				'request, which carries name, schema, and options; Song discards anything else without complaining.',
		);
	}

	// Schema body: the meta-schema allows no keys beyond these four.
	const extraKeys = Object.keys(schema).filter((k) => !ALLOWED_SCHEMA_KEYS.has(k));
	if (extraKeys.length > 0) {
		errors.push(
			`schema may only contain type, definitions, properties, and required; Song rejects ${quoteList(extraKeys)}.`,
		);
	}
	...
	if (Object.keys(properties).length === 0) {
		warnings.push('schema.properties is empty; add at least one property.');
	}

	// required: an array of unique strings, naming properties that exist.
	if ('required' in schema) {
		if (!Array.isArray(schema.required)) {
			errors.push('schema.required must be an array of property names.');
		} else {
			...
			const repeated = Array.from(new Set(required.filter((r, i) => required.indexOf(r) !== i)));
			if (repeated.length > 0) {
				errors.push(`schema.required repeats ${quoteList(repeated)}; its entries must be unique.`);
			}
			for (const name of required) {
				if (!(name in properties)) {
					warnings.push(`required lists "${name}", which is not defined in properties.`);
				}
			}
		}
	}
	...
	const reserved = Object.keys(properties).filter((key) => RESERVED_TOP_LEVEL.has(key));
	if (reserved.length > 0) {
		errors.push(`${quoteList(reserved)} belong to Song's base schema and cannot be redefined at the top level.`);
	}
	const reported = new Set(reserved);
	if ('experiment' in properties && !isPlainObject(properties.experiment)) {
		errors.push('"experiment" must be an object; Song\'s meta-schema constrains it.');
		reported.add('experiment');
	}

	collectWarnings(properties, '', warnings, 0, reported);
	...
	if (unresolved.size > 0) {
		errors.push(`unresolved $ref: ${quoteList(Array.from(unresolved))}; add it under "definitions".`);
	}
