...
3. **Update the `SCORE_ACCESSTOKEN` variable within your `.env.song`** and once updated, remove the existing Song container and re-run Song with your updated `.env.song`
    ...
    :::tip Next Steps
    Now that you have the end-to-end portal setup we recommend you check out our [administration guide on building Song schemas](/use/administration/building-song-schemas).
    :::
