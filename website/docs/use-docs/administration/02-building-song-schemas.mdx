...
## Try It: Song Schema Playground

You have seen the pieces; now assemble one. The playground below lets you build a Song schema two ways. Press **Generate from CSV** to turn example data into a starter schema, where each column becomes a typed property and low-cardinality columns get a suggested `enum`; or write the JSON directly in the editor. It validates the Song envelope and the inner JSON Schema as you type, flagging issues such as a `required` field that is not defined, an unknown type, or an invalid regex `pattern`, and it renders the fields on the right. Everything runs in your browser. Use **Download JSON** or **Copy JSON** to carry your schema to the update step below.
