...
## Try It: Generate Your Arranger Configs

Rather than assembling `base.json`, `extended.json`, `table.json`, and `facets.json` from scratch, generate a working set from your data and adjust from there. Give the generator below a CSV (upload a file, paste rows, or press **Load example**) and it produces all four Arranger configuration files: a `base.json` pointing at your index, an `extended.json` with a display name per field, a `table.json` with a sortable column per field, and a `facets.json` with an aggregation for each searchable field. Everything runs in your browser; nothing is sent to a server. Switch between the tabs to review each file, then copy or download it.

<ConfigGenerator
  outputs={['arrangerBase', 'arrangerExtended', 'arrangerTable', 'arrangerFacets']}
  title="Arranger Config Generator"
  subtitle="Turn a CSV into base.json, extended.json, table.json, and facets.json, entirely in your browser."
/>

The generated configs use flat, top-level fields (`data.<column>`), so they are an ideal starting point for a new index. When your Elasticsearch documents nest fields more deeply, extend the generated `fieldName` values using the `.` (and `__` for facets) notation described above.
