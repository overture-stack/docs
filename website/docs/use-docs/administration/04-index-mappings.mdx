...
You do not have to write an index template by hand to get started. The [Config Generator](/use/config-generators-and-playgrounds/config-generator) infers an index mapping from example data (the mapping format is shared by Elasticsearch and OpenSearch): give it a CSV, and it produces the `index_patterns`, `aliases`, `mappings`, and `settings` described above, with each column typed as `keyword`, `integer`, `float`, `boolean`, or `date` from its sample values. Use it as a starting point, then layer on the analyzers, custom settings, and nested structure your data model needs.
