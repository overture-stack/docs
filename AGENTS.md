...
- Build and serve: `cd website && npm run build && npm run serve --port <port>` for marketing, or the `:docs` variants for the documentation site. `npm start` renders in the browser, so its HTML is an empty shell and tells you nothing.
...
- Check a documentation page too whenever marketing styles or theme components change: the two sites are separate builds but one codebase, and `src/theme/` and the emitted stylesheet are shared by both. `npm run build` and `npm run build:docs` both have to pass.
...
- **Two sites, two builds, one branch.** `docusaurus.config.ts` reads `OVERTURE_SITE`: `marketing` builds overture.bio and is the default the plain `build`, `start` and `serve` scripts set, unset builds docs.overture.bio and is what the `:docs` variants do. The default is marketing so the overture.bio Netlify site runs `npm run build` into `website/build` with no environment variable of its own; docs.overture.bio names `npm run build:docs` and publishes `website/build-docs`. The marketing build drops the documentation plugin instances and the redirect table, and points the pages plugin at `src/marketing/pages/`. Both builds keep Algolia: `MarketingNavbar` renders the same `@theme/SearchBar`, and the marketing build adds `externalUrlRegex` so results navigate cross-host to docs.overture.bio instead of 404ing. Components ask which build they are in through `useIsMarketingSite`, never by inspecting the route.
- `website/static-docs/` and `website/static-marketing/` — what each host owns alone: `robots.txt` for both, and the `_redirects` file each one publishes. `website/static/` is still shared. There is deliberately no `netlify.toml`: one file on one branch cannot say different things to two Netlify sites, and each site is configured through the Netlify UI. Redirects belong in the `_redirects` file of the host that owes them.
